      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="rounded-2xl bg-white/10 px-3 py-2 text-right backdrop-blur">
            ...
            <div className="mt-1 text-lg font-semibold">{totalRecords}</div>
          </div>
        </div>
        <div className="mt-4 flex items-center justify-between rounded-2xl bg-white/10 px-4 py-3 backdrop-blur">
          <div>
            ...
            <div className="mt-1 text-xl font-semibold">{currency(totalAmount)}</div>
          </div>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
      <div className="mx-auto min-h-screen max-w-md bg-slate-100 pb-28">
        <div className="sticky top-0 z-20 bg-slate-100/90 px-4 pb-4 pt-4 backdrop-blur">
          <SummaryBanner
            role={role}
          ...
          <div className="mt-3">
            {role === "capo" ? (
              ...
              <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 shadow-sm">
                Profilo limitato: vedi solo le tue spese e puoi caricarne di nuove.
              </div>
            )}
          </div>
        </div>
        ...
        <nav className="fixed bottom-0 inset-x-0 mx-auto max-w-md border-t border-slate-200 bg-white/95 backdrop-blur">
          <div className={cn("grid", role === "capo" ? "grid-cols-5" : "grid-cols-3")}>
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => {
                ...
                className={cn(
                  ...
                  screen === item.id ? "text-slate-950" : "text-slate-500"
                )}
              >
                <span className={cn("text-base", screen === item.id && "scale-110")}>{item.emoji}</span>

...
export default function App() {
  ...
  const totals = useMemo(() => {
    ...
    const imponibile = records.reduce((sum, record) => sum + (Number(record.imponibile) || 0), 0);
    const vat = records.reduce((sum, record) => sum + (Number(record.vat) || 0), 0);
  ...
  const jobStats = useMemo(() => {
    return jobs
      .map((job) => {
        ...
        return {
          job,
          total: related.reduce((sum, record) => sum + (Number(record.amount) || 0), 0),
          imponibile: related.reduce((sum, record) => sum + (Number(record.imponibile) || 0), 0),
          vat: related.reduce((sum, record) => sum + (Number(record.vat) || 0), 0),
  ...
  const monthlySpend = useMemo(() => {
    ...
    records.forEach((record) => {
      ...
      map[key].primary += Number(record.amount) || 0;
      map[key].secondary += Number(record.vat) || 0;
    });
  ...
  const supplierBreakdown = useMemo(() => {
    const map = {};
    records.forEach((record) => {
      map[record.supplier] = (map[record.supplier] || 0) + (Number(record.amount) || 0);
    });
    return Object.entries(map)
      .map(([label, value]) => ({ label, value }))
      .sort((a, b) => b.value - a.value)
  ...
  const paymentBreakdown = useMemo(() => {
    const map = {};
    records.forEach((record) => {
      map[record.paymentMethod] = (map[record.paymentMethod] || 0) + (Number(record.amount) || 0);
    });
    return Object.entries(map)
      .map(([label, value]) => ({ label, value }))
      .sort((a, b) => b.value - a.value);
      ...
      <div className="mx-auto min-h-screen max-w-md bg-slate-100 pb-24">
        <div className="px-4 pb-3 pt-4">
          <SummaryBanner
            ...
            totalAmount={role === "capo" ? totals.total : myRecords.reduce((sum, record) => sum + (Number(record.amount) || 0), 0)}
          />
        </div>

        {role === "capo" && screen === "dashboard" ? (
        ...
        <main className="space-y-4 px-4 pb-4">
            ...
            <>
              ...
              {bossViewMode === "advanced" ? (
                <>
                  ...
                  <HorizontalChart title="Metodi di pagamento" subtitle="Distribuzione tra bonifico, carta, contanti e conto." data={paymentBreakdown} />
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <StatCard
                  ...
                  value={currency(myRecords.reduce((sum, record) => sum + (Number(record.amount) || 0), 0))}
                  hint="Somma delle tue spese"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <QuickActionCard icon="➕" title="Carica spesa" subtitle="Aggiungi subito un nuovo documento." onClick={() => setScreen("add")} />
                <QuickActionCard icon="📄" title="Le mie spese" subtitle="Apri l’archivio filtrato sul tuo utente." onClick={() => setScreen("archive")} />
              </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <StatCard
                  ...
                  value={currency(filteredRecords.reduce((sum, record) => sum + (Number(record.amount) || 0), 0))}
              ...
              {role === "capo" && archiveViewMode === "advanced" ? (
                <div className="grid grid-cols-2 gap-3">
                  <StatCard label="IVA visibile" value={currency(filteredRecords.reduce((sum, record) => sum + (Number(record.vat) || 0), 0))} />
                  <StatCard label="Imponibile visibile" value={currency(filteredRecords.reduce((sum, record) => sum + (Number(record.imponibile) || 0), 0))} />
                </div>
              ) : null}
            ...
            <>
              ...
              <Card className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3 pt-1">
                  ...
                  <SecondaryButton onClick={() => setForm({ ...form, amount: "", vat: "", description: "", file: "" })}>
                    Pulisci
                  </SecondaryButton>
                </div>
              </Card>
              ...
              <Card className="p-4">
                ...
                <div className="mt-4 space-y-3">
                  {records
                    ...
                    .map((record) => (
                      <ExpenseCard key={record.id} record={record} onOpen={setSelectedRecord} onOpenJob={openJob} canOpenJob showMeta />
                    ))}
                </div>
              </Card>
            </>
          ...
          {screen === "users" && role === "capo" && (
            <>
              <HeaderBlock eyebrow="Permessi" title="Utenti e ruoli" subtitle="Una schermata più leggibile per capire chi può fare cosa." />

              <Card className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-base font-semibold text-slate-900">Titolare / Admin</div>
                    <div className="mt-1 text-sm text-slate-500">Accesso completo a dashboard, archivio, cantiere, lavorazioni e dettagli.</div>
                  </div>
              ...
              <Card className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-slate-500">Può caricare spese e vedere soltanto i documenti caricati da lui.</div>
                  </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    setSelectedRecord(null);
                    if (role === "dipendente" && ["site", "users", "job"].includes(item.id)) return;
                    setScreen(item.id);
            ...
            <div className="mx-auto mt-8 max-w-md">
              <Card className="max-h-[85vh] overflow-hidden">
                ...
                <div className="max-h-[calc(85vh-92px)] overflow-y-auto px-5 py-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-1">
                    <PrimaryButton onClick={() => setSelectedRecord(null)}>Apri file</PrimaryButton>
                    {role === "capo" ? (
                      <SecondaryButton
                        onClick={() => {
